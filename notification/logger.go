...
func (fileLogger FileLogger) log(message string, severity string) {
	f, err := os.OpenFile("logs/notification.log", os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0644)
	if err != nil {
		log.Panicln(err)
	}
	...
	fileLogger.logger = log.New(f, fmt.Sprintf("notification.%s ", severity), log.LstdFlags)
